...
public class Knight : Unit
{
	...
	void Update()
	{
		...
		//		}
		//		
		//Here is how the mechanism will work: Chosen when Knight created is false. When you click on Knight, chosen is always true. 
		//If you right click, chosen = false. If you click on a tile when chosen is true, Knight will attempt to move there.
		if (Chosen && Physics.Raycast(ray, out hit, 1000) && Input.GetMouseButtonDown(1) && hit.transform != transform) //attempt to move Knight
		{
			...
			CurrentTile = transform.GetComponentInParent<Tile>(); //reset the currentTile to the tile this script is on. Do this even if the Knight OBJECT (in game) is not on this tile. 
		}
	}
	...
	public void Invade(NetworkViewID destinationID)
	{
		...
		if (Network.isServer)
		{
			...
			else if (_neutral)
			{
				...
				{
					CurrentTile.updateBool("HasKnight", false);
					CurrentTile.updateBool("HasUnit", false);
					destination.updateBool("HasKnight", true);
					CurrentTile = destination; //now the little fucker's destination tile is on the empty square!
					destination.Knight.transform.GetComponent<Knight>().updateBool("canMove", false);
					Add(CurrentTile);
				}
				...
				// Test case 5: tile has a tombstone, move Knight there, but can't move again
			...
			else if (_enemy) //can invade enemy tiles that have a peasant or non-villager tiles
			{
				foreach (Transform neighborTransform in destination.Neighbours)
				{
					...
					// Check to see whether the destination tile is guarded by Knight or higher villager (not peasant).
					...
					if ((neighborTransform.GetComponent<Renderer>().material.color == destination.transform.GetComponent<Renderer>().material.color
					    && neighborTile.HasUnit == true && neighborTile.HasPeasant == false && neighborTile.HasInfantry == false && neighborTile.HasSoldier == false && neighborTile.HasCannon == false) || neighborTile.HasCastle)
					{
				...
				foreach (Tile destination in list)
				{
					...
					if (_home)
					{
						...
						if (destination.IsEmpty)
						{
							...
							CurrentTile.updateBool("HasUnit", false);
							destination.updateBool("HasKnight", true);
							destination.Knight.transform.GetComponent<Knight>().updateBool("canMove", true);
							CurrentTile = destination;
